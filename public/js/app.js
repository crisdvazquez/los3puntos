...
// Refresco por fetch, usado solo para vistas de liga específica (no HOME)
async function refrescarEnVivo() {
    if (ligaActual === 'HOME') return;
...
// Conexión única al stream de partidos en vivo (reemplaza el polling por timer)
function conectarStreamEnVivo() {
    if (eventSourceEnVivo) return;

    eventSourceEnVivo = new EventSource('/api/live/stream');
    eventSourceEnVivo.onmessage = (e) => {
        try {
            const payload = JSON.parse(e.data);
            manejarActualizacionEnVivo(payload.events || []);
        } catch (err) {
            // ignorar mensajes mal formados
        }
    };
    eventSourceEnVivo.onerror = () => {
        // EventSource reconecta solo; no hace falta manejo manual acá
    };
}

function manejarActualizacionEnVivo(rawEvents) {
    if (ligaActual === 'HOME' && homeOffsetDias === 0) {
        const liveScoresFormato = rawEvents.map(ev => ({
            fixtureId: ev.fixtureId,
            homeScore: ev.intHomeScore,
            awayScore: ev.intAwayScore,
            displayMinute: ev.displayMinute
        }));
        const actualizado = aplicarLiveScoresHome(liveScoresFormato);
        if (!actualizado && liveScoresFormato.length > 0) {
            actualizarPartidosHome({ background: true });
        }
    } else if (ligaActual !== 'HOME') {
        refrescarEnVivo();
    }
}
